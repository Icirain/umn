...
StrategyGetBuffer(BufferAccessStrategy strategy, bool *lock_held)
{
	...
	Latch	   *bgwriterLatch;
	int			trycounter;
	static int timestamp[1000000]={0};
	static int stamp=0;
	int i;
	...
	if (strategy != NULL)
	{
		...
		if (buf != NULL)
		{
			*lock_held = false;
			stamp=stamp+1;
			timestamp[buf->buf_id]=stamp;
			//printf("#1  stamp: %d    number_replace:%d\n",stamp,number_replace);
			return buf;
		}
	}
	...
	*lock_held = true;
	...
	while (StrategyControl->firstFreeBuffer >= 0)
	{
		...
		if (buf->refcount == 0 && buf->usage_count == 0)
		{
			...
			stamp=stamp+1;
			timestamp[buf->buf_id]=stamp;
			//printf("#2  stamp: %d    number_replace:%d\n",stamp,number_replace);
			return buf;
		}
		UnlockBufHdr(buf);
	}
   ...
   if(LIFO)
	 {
		 ...
		 if(buf->refcount==0)
		 {
			 ...
			 {
				 if(timestamp[buf->buf_id]>timestamp[last->buf_id])
				 {
					 last=buf;
				 }
			 }
		 ...
		 if(trycounter==0)
		 {
			 ...
			 if(last!=NULL)
			 {
				 printf("Replaced buffer: %d\n\n",timestamp[last->buf_id]);
				 stamp++;
				 timestamp[last->buf_id]=stamp;
				 if(strategy!=NULL)
				   AddBufferToRing(strategy,last);
				 LockBufHdr(last);
				 return last;
			 }
			 else
